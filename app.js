...
/* =========================================================
   HERO CAROUSEL — 10 SLIDES / 5 VISIBLE DOT WINDOW
   Dots stay at exactly 5 on mobile. The window moves only
   when the active slide reaches an edge.
========================================================= */
...
function updateHero() {
  ...
  hero.querySelector(".hero-badge").textContent = `🔥 #${heroIndex + 1} TRENDING`;
  hero.querySelector("h1").textContent = anime.title;
  hero.querySelector(".hero-meta").innerHTML = `⭐ ${anime.rating} &nbsp; • &nbsp; ${anime.year} &nbsp; • &nbsp; ${anime.type} &nbsp; • &nbsp; ${anime.episodes ? anime.episodes + (anime.episodes >= 100 ? "+ Episodes" : " Episodes") : "Movie"}`;
  hero.querySelector(".hero-description").textContent = anime.description || `Watch ${anime.title} on Anime Fan.`;
...
function moveHero(direction) {
  ...
  // Forward: move the 5-dot window only after the last visible dot is crossed.
  if (direction > 0 && heroIndex > heroDotStart + visibleDots - 1) {
    heroDotStart = Math.min(heroDotStart + 1, maxStart);
  }

  // Backward: keep the current 5-dot window until the first visible dot is crossed.
...
function initHeroCarousel() {
  ...
  hero.addEventListener("pointerup", event => {
    ...
    mouseStartX = null;
    if (Math.abs(dx) >= 50) moveHero(dx < 0 ? 1 : -1);
  });
}
